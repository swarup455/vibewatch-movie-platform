    ...
    return (
        <main className="flex justify-center px-4 pt-6">
            <div className="w-full max-w-6xl">
                ...
                <div className="flex items-center justify-end gap-2 mb-16">
                    <Button
                        variant="outline"
                        size="sm"
                ...
                {/* Search box */}
                <div className="relative rounded-2xl border border-border bg-background/60 backdrop-blur-sm shadow-sm p-4">
                    <Textarea
                        value={input}
                        onChange={(e) => setInput(e.target.value)}
                        onKeyDown={handleKeyDown}
                        placeholder="e.g. Mind-bending sci-fi, something relaxing before sleep, anime like Attack on Titan..."
                        className="min-h-[100px] resize-none border-0 bg-transparent p-0 text-sm text-foreground placeholder:text-muted-foreground focus-visible:ring-0 focus-visible:ring-offset-0"
                    />
